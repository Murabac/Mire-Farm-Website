...
-- Benefits Table
CREATE TABLE IF NOT EXISTS mire_farm_website.benefits (
...
ALTER TABLE mire_farm_website.benefits ENABLE ROW LEVEL SECURITY;

-- Create policy for public read access
CREATE POLICY "Benefits are viewable by everyone"
  ON mire_farm_website.benefits FOR SELECT
  USING (true);

-- Create index for display order and active status
CREATE INDEX IF NOT EXISTS idx_benefits_display_order ON mire_farm_website.benefits(display_order);
CREATE INDEX IF NOT EXISTS idx_benefits_active ON mire_farm_website.benefits(active);

-- Insert Benefits Data with English, Somali, and Arabic translations
INSERT INTO mire_farm_website.benefits (text_en, text_so, text_ar, display_order, active) VALUES

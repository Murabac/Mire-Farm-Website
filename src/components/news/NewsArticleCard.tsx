    ...
    <Link href={`/news/${article.id}`}>
      <article className="bg-gradient-to-br from-white to-gray-50 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1 border-2 border-gray-100 cursor-pointer">
        ...
        <div className="md:col-span-3 p-5 md:p-6">
          ...
          <h2 className="text-xl md:text-2xl lg:text-3xl text-[#2C5F2D] mb-3 md:mb-4 leading-tight">{article.title}</h2>
          <p className="text-base md:text-lg text-gray-700 mb-3 md:mb-4 leading-relaxed">{article.excerpt}</p>
          <p className="text-sm md:text-base text-gray-600 leading-relaxed">{article.content}</p>
        </div>
      </div>
    </article>
    </Link>

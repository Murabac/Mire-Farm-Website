...
export default function ArticleClient({ article, relatedArticles }: ArticleClientProps) {
  ...
  // Localize the article
  const localizedArticle = getLocalizedNewsArticle(article, language);
  
  // Localize related articles
  const localizedRelatedArticles = relatedArticles.map(rel => getLocalizedNewsArticle(rel, language));

  return (
    <main>
      <PageHeader
        badge={{
          text: localizedArticle.badge,
        }}
        title={localizedArticle.title}
        description={
          <>
            <span className="text-[#6B9E3E] font-semibold">{localizedArticle.excerpt}</span>
          </>
        }
      />
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            <div className="flex items-center gap-2 bg-white border-2 border-gray-200 text-gray-700 px-4 py-2 rounded-full">
              <span className="text-xl">{localizedArticle.emoji}</span>
              <span className="font-medium">{localizedArticle.badge}</span>
            </div>
          </div>
